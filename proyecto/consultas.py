...
collection_anime1 = db['anime']

pipeline_most_episodes = [
    {
        '$match': {
            'Episodes': {'$ne': 'Unknown'},  # Filtrar documentos donde Episodes no sea 'Unknown'
            'English name': {'$ne': 'Unknown'}  # Filtrar documentos donde English name no sea 'Unknown'
        }
    },
    {
        '$addFields': {
            'EpisodesInt': {'$toInt': '$Episodes'}  # Convertir Episodes a integer
        }
    },
    {
        '$sort': {'EpisodesInt': -1}  # Ordenar por número de episodios en orden descendente
    },
    {
...
# Ejecutar la consulta y obtener los resultados
result_most_episodes = list(collection_anime1.aggregate(pipeline_most_episodes))
